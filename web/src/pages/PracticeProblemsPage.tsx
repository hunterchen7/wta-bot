...
export function PracticeProblemsPage() {
  ...
  return <div className="space-y-7">
    ...
    {data.problems.length ? <div className="space-y-8">
      ...
      {data.problems.filter((problem) => problem.round === period.round).map((problem) => <details
        key={problem.id}
        className="group overflow-hidden rounded-2xl border border-border bg-card shadow-sm open:shadow-md"
      >
        <summary className="flex cursor-pointer list-none items-center gap-4 p-5 transition hover:bg-muted/50 marker:content-none sm:p-6">
          <div className="min-w-0 flex-1">
            ...
            <h3 className="text-lg font-black text-foreground group-open:text-western-800 dark:group-open:text-western-300">{problem.title}</h3>
          </div>
