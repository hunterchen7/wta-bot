  ...
  return <div className="space-y-5">
    ...
    <div className="grid gap-4 xl:grid-cols-[minmax(0,1.25fr)_minmax(20rem,.75fr)]">
      <Panel title="Action required" description="Queues are ordered by operational impact.">
        {activeQueues.length ? <div className="divide-y divide-slate-100">{activeQueues.map(([key, label, href]) => {
          const value = data.queues[key];
          return <Link key={key} to={href} className="group flex items-center gap-4 px-5 py-3 hover:bg-slate-50/80 dark:hover:bg-white/5"><span className="grid size-9 shrink-0 place-items-center rounded-xl bg-amber-100 text-sm font-black tabular-nums text-amber-800">{value}</span><span className="min-w-0 flex-1"><span className="block text-sm font-bold text-slate-900">{label}</span><span className="block text-xs text-slate-500">Open the queue and resolve</span></span><span className="text-slate-300 transition group-hover:translate-x-0.5 group-hover:text-slate-600">→</span></Link>;
        })}</div> : <div className="flex items-center gap-3 p-5"><span className="grid size-9 shrink-0 place-items-center rounded-xl bg-emerald-50 font-black text-emerald-700">✓</span><div><div className="text-sm font-bold text-slate-900">Nothing needs attention</div><div className="text-xs text-slate-500">All operational queues are clear.</div></div></div>}
      </Panel>
