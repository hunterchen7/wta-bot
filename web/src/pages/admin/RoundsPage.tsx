  ...
  return <Panel title="Opt-in pool" description="Round opt-ins, late first-come-first-served entries, and organizer-added interviewer capacity.">
    ...
    {data.optins.length ? <div className={tableWrapClass}><table className={tableClass}>
      ...
      <tbody>{data.optins.map((row) => <tr key={row.participant_id}>
        ...
        <td className={tdClass}><div className="flex flex-wrap gap-2">{row.regular_opt_in === 1 ? <Badge value="opted in" /> : <Badge value="extra only" />}{row.standby ? <Badge value={`standby · ${Number(row.standby_interviewer_limit ?? 0)} interviewer / ${Number(row.standby_interviewee_limit ?? 0)} interviewee`} /> : null}{row.wants_double ? <Badge value="double" /> : null}{row.extra_interviewer ? <Badge value="extra interviewer" /> : null}</div></td>
