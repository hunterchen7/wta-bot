...
function DetailRow({ label, value, link, href }: { label: string; value?: string | null; link?: 'email' | 'url'; href?: string }) { const destination = href ?? (link === 'email' && value ? `mailto:${value}` : link === 'url' && value ? value : null); const content = value || 'Not provided'; return <div className="grid grid-cols-[8rem_minmax(0,1fr)] gap-3 py-2.5 text-sm first:pt-0 last:pb-0"><dt className="text-slate-500">{label}</dt><dd className="min-w-0 break-words font-semibold text-slate-800">{destination ? <a className="text-indigo-600 hover:underline dark:text-indigo-300" href={destination} target={link === 'email' ? undefined : '_blank'} rel={link === 'email' ? undefined : 'noreferrer'}>{content}</a> : content}</dd></div>; }
