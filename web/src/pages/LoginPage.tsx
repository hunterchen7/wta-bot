...
export function LoginPage() {
  ...
  if (checkingSession) return <PublicShell narrow><div className="mx-auto grid min-h-[50vh] place-items-center" role="status" aria-live="polite"><div className="text-center"><div className="mx-auto size-2 animate-pulse rounded-full bg-western-500 shadow-[0_0_0_7px_rgba(143,85,224,.16)]" /><p className="mt-5 text-sm font-bold text-slate-500">Checking your session…</p></div></div></PublicShell>;
  ...
  return <PublicShell narrow><div className="mx-auto max-w-xl">
    ...
    <section className="rounded-3xl border border-slate-200 bg-white p-6 shadow-[0_18px_50px_rgba(15,23,42,.06)] sm:p-8">
      ...
      {step === 'email' ? <form onSubmit={submitEmail}><label className="text-sm font-extrabold text-slate-800">Preferred email<input autoFocus required type="email" value={email} onChange={(event) => setEmail(event.target.value)} placeholder="you@example.com" className={`${publicInputClass} mt-2`} /></label><button disabled={busy} className="mt-5 w-full cursor-pointer rounded-xl bg-western-700 px-4 py-3 text-sm font-black text-white transition hover:bg-western-800 disabled:cursor-wait disabled:opacity-60">{busy ? 'Sending…' : 'Send sign-in code'}</button></form>
      : <form onSubmit={submitCode}><label className="text-sm font-extrabold text-slate-800">Six-digit code<input ref={codeRef} required inputMode="numeric" pattern="[0-9]{6}" maxLength={6} value={code} onChange={(event) => setCode(event.target.value.replace(/\D/g, '').slice(0, 6))} placeholder="123456" className={`${publicInputClass} mt-2 text-center font-mono text-2xl tracking-[0.35em]`} /></label><button disabled={busy || code.length !== 6} className="mt-5 w-full cursor-pointer rounded-xl bg-western-700 px-4 py-3 text-sm font-black text-white transition hover:bg-western-800 disabled:cursor-not-allowed disabled:opacity-50">{busy ? 'Checking…' : 'Open dashboard'}</button><button type="button" onClick={() => { setStep('email'); setCode(''); setError(''); }} className="mt-3 w-full cursor-pointer px-4 py-2 text-sm font-bold text-slate-500 hover:text-slate-900">Use a different email</button></form>}
    </section>
