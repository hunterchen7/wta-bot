...
export type ReportField = { id: string; label: string; type: 'radio' | 'select' | 'scale' | 'text' | 'textarea' | 'url'; options?: Array<{ value: string; label: string }>; readOnly?: boolean; required?: boolean; shared?: boolean; help?: string; mono?: boolean; low?: string; high?: string; placeholder?: string; noShowOnly?: boolean };
...
function ReportFieldInput({ field, value, error, index, token, preview, onChange }: { field: ReportField; value: string; error?: string; index: number; token?: string; preview: boolean; onChange: (value: string) => void }) {
  ...
  return <div data-report-field={field.id} className={`${shell} block`}>{head}<div className="mt-4">{field.id === 'video_url' ? <VideoUploadField token={token} value={value} preview={preview} invalid={Boolean(error)} onChange={onChange} /> : field.readOnly ? <div aria-readonly="true" className="rounded-xl border border-western-200 bg-western-50/70 px-4 py-3 text-sm font-bold text-slate-800 dark:border-western-800 dark:bg-western-950/30 dark:text-slate-100">{field.options?.find((option) => option.value === value)?.label ?? value}</div> : field.type === 'select' ? <SelectControl label={field.label} required={field.required} value={value} onChange={onChange} options={field.options ?? []} placeholder="Choose an option…" invalid={Boolean(error)} className="h-11" /> : field.type === 'textarea' && field.mono ? <CodeEditor value={value} invalid={Boolean(error)} onChange={onChange} /> : field.type === 'textarea' ? <Textarea aria-invalid={Boolean(error)} required={field.required} rows={5} value={value} placeholder={field.placeholder} onChange={(event) => onChange(event.target.value)} className="min-h-32 resize-y rounded-xl bg-background" /> : <Input aria-invalid={Boolean(error)} required={field.required} type={field.type === 'url' ? 'url' : 'text'} value={value} onChange={(event) => onChange(event.target.value)} className="h-11 rounded-xl bg-background" />}</div>{alert}</div>;
}
