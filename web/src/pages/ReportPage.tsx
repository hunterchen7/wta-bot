...
export function ReportPage({ previewKind }: { previewKind?: string }) {
  ...
  return <ReportShell embedded={embedded}>
    ...
    <form onSubmit={submit} className="space-y-4">{data.fields.map((field, index) => <ReportFieldInput key={field.id} field={field} value={values[field.id] ?? ''} language={values.language ?? ''} error={fieldErrors[field.id]} index={index + 1} onChange={(value) => set(field.id, value)} onLanguageChange={(value) => set('language', value)} />)}
...
function ReportFieldInput({ field, value, language, error, index, onChange, onLanguageChange }: { field: ReportField; value: string; language: string; error?: string; index: number; onChange: (value: string) => void; onLanguageChange: (value: string) => void }) {
  ...
  return <div data-report-field={field.id} className={`${shell} block`}>{head}<div className="mt-4">{field.type === 'select' ? <Select required={field.required} value={value || undefined} onValueChange={onChange}><SelectTrigger aria-invalid={Boolean(error)} className="h-11 w-full cursor-pointer rounded-xl bg-background"><SelectValue placeholder="Choose an option…" /></SelectTrigger><SelectContent position="popper" align="start" className="min-w-[var(--radix-select-trigger-width)]">{field.options?.map((option) => <SelectItem key={option.value} value={option.value} className="cursor-pointer py-2.5">{option.label}</SelectItem>)}</SelectContent></Select> : field.type === 'textarea' && field.mono ? <CodeEditor value={value} language={language} invalid={Boolean(error)} onChange={onChange} onLanguageDetected={onLanguageChange} /> : field.type === 'textarea' ? <Textarea aria-invalid={Boolean(error)} required={field.required} rows={5} value={value} onChange={(event) => onChange(event.target.value)} className="min-h-32 resize-y rounded-xl bg-background" /> : <Input aria-invalid={Boolean(error)} required={field.required} type={field.type === 'url' ? 'url' : 'text'} value={value} onChange={(event) => onChange(event.target.value)} className="h-11 rounded-xl bg-background" />}</div>{alert}</div>;
}
