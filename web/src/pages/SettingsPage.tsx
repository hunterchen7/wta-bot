...
export function SettingsPage() {
  ...
  const [form, setForm] = useState<SettingsPayload>(() =>
    fromParticipant(data.participant),
  );
  ...
  const submit = async (event: FormEvent) => {
    ...
    try {
      ...
      setMessage({
        kind: "ok",
        text:
          form.emailOk && !data.participant.emailOk
            ? "Settings saved. Check your inbox for the email opt-in confirmation."
            : "Settings saved.",
      });
      await refresh();
    } catch (cause) {
    ...
    <div className="space-y-6">
      <section>
        ...
        <p className="mt-2 text-slate-600">
          Update your profile and notification preferences, then save everything
          together.
        </p>
      </section>
      ...
      {message ? (
        <div
          className={`rounded-2xl border p-4 font-semibold ${message.kind === "ok" ? "border-emerald-200 bg-emerald-50 text-emerald-900" : "border-rose-200 bg-rose-50 text-rose-900"}`}
        >
          {message.text}
        </div>
      ) : null}
        ...
        >
          <div className="grid gap-5 sm:grid-cols-2">
            ...
            >
              <input
                name="preferredEmail"
                aria-invalid={Boolean(fieldErrors.preferredEmail)}
                required
                type="email"
                maxLength={200}
          ...
          <div className="mt-6 space-y-5">
            ...
            >
              <textarea
                name="blurb"
                aria-invalid={Boolean(fieldErrors.blurb)}
                required
                maxLength={2000}
                rows={8}
                value={form.blurb}
            ...
            >
              <textarea
                name="priorFeedback"
                aria-invalid={Boolean(fieldErrors.priorFeedback)}
                maxLength={1000}
                rows={4}
        ...
        <div className="fixed inset-0 z-50 grid place-items-center bg-slate-950/50 p-4 backdrop-blur-sm">
          ...
          >
            ...
            <p className="mt-2 leading-6 text-slate-600">
              Your settings have changed but have not been saved yet.
            </p>
...
function Field({
  ...
  return (
    <label className="block">
      <span className="mb-2 block text-sm font-bold text-slate-700 sm:min-h-10">
        {label}
      </span>
      {children}
      {help ? (
...
function ChoiceGroup({
  ...
  return (
    <fieldset
      data-field={field}
      tabIndex={-1}
      className={`mt-6 rounded-2xl outline-none ${error ? "ring-2 ring-rose-300 ring-offset-4" : ""}`}
    >
...
const omit = (values: Record<string, string>, key: PropertyKey) =>
  Object.fromEntries(Object.entries(values).filter(([entry]) => entry !== key));
const wordCount = (value: string) =>
  value.trim().split(/\s+/).filter(Boolean).length;

const fromParticipant = (
  participant: ReturnType<typeof useDashboard>["data"]["participant"],
): SettingsPayload => ({
