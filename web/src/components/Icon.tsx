...
export type IconName = 'analytics' | 'chevron' | 'close' | 'collapse' | 'forms' | 'grip' | 'mcp' | 'menu' | 'operations' | 'overview' | 'participants' | 'problems' | 'progress' | 'reviews' | 'rounds' | 'settings' | 'sort' | 'sortAsc' | 'sortDesc';
...
export function Icon({ name, className = 'size-5' }: { name: IconName; className?: string }) {
  const paths: Record<IconName, ReactNode> = {
    ...
    sortAsc: <><path d="m8 14 4-4 4 4" /><path d="M12 10v9" /></>,
    sortDesc: <><path d="m8 10 4 4 4-4" /><path d="M12 5v9" /></>,
  };
