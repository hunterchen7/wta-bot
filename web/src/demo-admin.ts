...
const problems: ProblemsData['problems'] = [
  { id: 1, source: 'leetcode', number: 56, title: 'Merge Intervals', url: 'https://leetcode.com/problems/merge-intervals/', difficulty: 'medium', difficulty_rank: 2.3, statement_md: 'Given an array of intervals, merge overlapping intervals.', hints_md: 'Sort by start time.\nTrack the current merged interval.', solution_md: 'Sort, then scan once.', active: 1, uses: 8, exposures: 16 },
  { id: 2, source: 'leetcode', number: 20, title: 'Valid Parentheses', url: 'https://leetcode.com/problems/valid-parentheses/', difficulty: 'easy', difficulty_rank: 1.1, statement_md: 'Determine whether brackets are balanced.', hints_md: 'Use a stack.', solution_md: 'Push openers and match closers.', active: 1, uses: 6, exposures: 12 },
  { id: 3, source: 'leetcode', number: 200, title: 'Number of Islands', url: 'https://leetcode.com/problems/number-of-islands/', difficulty: 'medium', difficulty_rank: 2.6, statement_md: 'Count connected land components.', hints_md: 'Try DFS or BFS.', solution_md: 'Flood-fill each unseen land cell.', active: 1, uses: 5, exposures: 10 },
  { id: 4, source: 'manual', number: null, title: 'Legacy Graph Exercise', url: null, difficulty: 'hard', difficulty_rank: 3.2, statement_md: null, hints_md: null, solution_md: null, active: 0, uses: 0, exposures: 0 },
];

const operations: OperationsData = {
  outbox: [
    { id: 203, kind: 'email', attempts: 5, run_after: now, done_at: null, last_error: 'Email binding rejected recipient', created_at: now },
    { id: 202, kind: 'dm', attempts: 1, run_after: '2026-08-13T14:05:00.000Z', done_at: null, last_error: 'Discord API 429', created_at: now },
    { id: 201, kind: 'channel_msg', attempts: 0, run_after: now, done_at: '2026-08-13T13:59:00.000Z', last_error: null, created_at: now },
  ],
...
export async function adminDemoRequest(path: string, init?: RequestInit): Promise<unknown> {
  ...
  if (path.match(/^\/participants\/\d+$/)) {
    ...
    return { participant: { ...participant, opportunities: '["internships","new_grad"]', topics: '["dsa","system_design"]', experience_band: '3-4', blurb: 'Interested in infrastructure and developer tools.' }, sessions: sessions.slice(0, 3), incidents: participant.strikes ? [{ id: 1, kind: 'unresponsive', state: 'confirmed', created_at: now, reporter_name: 'Jordan Lee' }] : [], audit: overview.recentAudit };
  }
  ...
  if (path === '/reviews') return { reviews: [
    { id: 71, review_state: 'pending', state: 'completed', round: 3, interviewer_name: 'Jordan Lee', interviewee_name: 'Maya Singh', interviewee_id: 3, video_url: 'https://example.com/recording', verdict: 'pass', verdict_reason: 'Clear communication and a correct solution with one small hint.' },
    { id: 70, review_state: 'flagged', state: 'completed', round: 3, interviewer_name: 'Alex Chen', interviewee_name: 'Sam Wilson', interviewee_id: 4, video_url: null, verdict: 'pass', verdict_reason: 'Recording link is inaccessible.' },
    { id: 68, review_state: 'verified', state: 'completed', round: 3, interviewer_name: 'Taylor Kim', interviewee_name: 'Priya Patel', interviewee_id: 6, video_url: 'https://example.com/recording2', verdict: 'pass', verdict_reason: 'Independent, well-explained solution.' },
  ] } satisfies ReviewsData;
  if (path === '/problems') return { problems, sets: problems.slice(0, 3).map((problem) => ({ week_id: 2, round: 2, cohort_name: cohort.name, problem_id: problem.id, title: problem.title })), cohort, weeks, bands: { 1: [1, 1.9], 2: [2, 2.4], 3: [2.5, 3.1] } } satisfies ProblemsData;
