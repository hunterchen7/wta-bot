export type Env = {
  DB: D1Database;
  RECORDINGS?: R2Bucket;
  AI?: Ai;
  ASSETS?: Fetcher;
  // Secrets — optional so the Worker boots before configuration; handlers guard.
  DISCORD_APP_ID?: string;
  DISCORD_PUBLIC_KEY?: string;
  DISCORD_TOKEN?: string;
  FORM_SIGNING_SECRET?: string;
  // Shared only with the Olares transcription worker. It grants access to the
  // narrow analysis job/media endpoints, never to R2 credentials directly.
  ANALYSIS_WORKER_SECRET?: string;
  // The app is public on Discord's side; this allowlist is the real gate.
  // Comma-separated guild ids. Unset/empty = allow all (pre-setup only).
  // Foreign-guild interactions get an ephemeral refusal + the bot leaves.
  ALLOWED_GUILD_IDS?: string;
  // Public base URL for links minted outside a request context (cron DMs).
  PUBLIC_ORIGIN?: string;
  // Comma-separated emails that get organizer views on the web dashboard
  // (checked at login, alongside the Discord organizer-role fallback).
  DASHBOARD_ADMINS?: string;
  // Outbox rows drained per tick (Discord/email sends). Defaults to 100 for
  // the paid Worker deployment; keep this bounded to respect provider limits.
  OUTBOX_BUDGET?: string;
  // Email Service binding (optional until the domain is onboarded in the dash).
  EMAIL?: {
    send(message: {
      to: string | { email: string; name?: string };
      from: string | { email: string; name?: string };
      subject: string;
      text?: string;
      html?: string;
    }): Promise<{ messageId?: string } | void>;
  };
  // Sender address — must belong to the onboarded Email Service domain.
  EMAIL_FROM?: string;
  // Display name shown to recipients (defaults to "Western Tech Alumni").
  EMAIL_FROM_NAME?: string;
};
