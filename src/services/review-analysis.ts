...
import { aiReviewSchema, normalizeAiReview, interviewRoleSchema, type AiReview } from './review-rubric-v3';
...
export async function reviewAnalysisForSession(env: Env, sessionId: number) {
  const audit = await activeReviewAudit(env, sessionId);
  const row = await env.DB.prepare(
    `SELECT * FROM review_analysis_jobs WHERE session_id = ?1 AND (?2 IS NULL OR id = ?2) ORDER BY created_at DESC, id DESC LIMIT 1`,
  ).bind(sessionId, audit?.run?.job_id ?? null).first<AnalysisJobRow>();
  if (!row) return null;
  const evaluationKey = audit ? audit.run?.evaluation_object_key : row.evaluation_object_key;
  const transcriptKey = audit ? audit.run?.transcript_object_key : row.transcript_object_key;
  let evaluation: AiReview | AiReviewV4 | null = null;
  let transcript: ReviewTranscript | null = null;
  if (env.RECORDINGS) {
    const [evaluationObject, transcriptObject] = await Promise.all([
      (audit ? audit.run?.status === 'ready' : row.status === 'ready') && evaluationKey
        ? env.RECORDINGS.get(evaluationKey)
        : Promise.resolve(null),
      transcriptKey
        ? env.RECORDINGS.get(transcriptKey)
        : Promise.resolve(null),
    ]);
    if (evaluationObject && evaluationObject.size <= MAX_TRANSCRIPT_BYTES) {
      try {
        const json: unknown = JSON.parse(await evaluationObject.text());
        if (audit?.batch.rubric_version === 'round3-review-v4') {
          const parsed = aiReviewV4Schema.safeParse(json);
          if (parsed.success) evaluation = normalizeAiReviewV4(parsed.data);
        } else {
          const parsed = aiReviewSchema.safeParse(json);
          if (parsed.success && (!audit || parsed.data.rubricVersion === audit.batch.rubric_version)) evaluation = normalizeAiReview(parsed.data);
        }
      } catch {
  ...
  return {
    id: row.id,
    status: audit ? (audit.run?.status === 'ready' ? 'ready' : 'queued') : row.status,
    auditBatchId: audit?.batch.id ?? null,
    evaluationRunId: audit?.run?.id ?? null,
    rubricVersion: audit?.batch.rubric_version ?? evaluation?.rubricVersion ?? row.rubric_version,
    transcriptionModel: row.transcription_model,
    diarizationModel: row.diarization_model,
    evaluatorModel: audit?.batch.evaluator_model ?? row.evaluator_model,
    reasoningEffort: audit?.batch.reasoning_effort ?? null,
    serviceTier: audit?.batch.service_tier ?? null,
    transcriptConfidence: row.transcript_confidence,
    speakerConfidence: row.speaker_confidence,
    lastError: audit ? (audit.run ? null : 'This session is not included in the active audit batch.') : row.last_error,
    createdAt: row.created_at,
    startedAt: row.started_at,
    transcribedAt: row.transcribed_at,
    evaluatedAt: audit ? audit.run?.completed_at ?? null : row.evaluated_at,
    captionsUrl: row.captions_object_key && (!audit || audit.run) ? `/api/admin/reviews/${sessionId}/captions` : null,
    transcriptUrl: transcriptKey ? `/api/admin/reviews/${sessionId}/transcript` : null,
    transcript,
    evaluation,
  };
}
...
export async function reviewAnalysisArtifact(
  ...
  if (!env.RECORDINGS) return null;
  const audit = await activeReviewAudit(env, sessionId);
  if (audit && kind === 'transcript') return audit.run?.transcript_object_key ? env.RECORDINGS.get(audit.run.transcript_object_key) : null;
  if (audit && !audit.run) return null;
  ...
  const row = await env.DB.prepare(
    `SELECT ${column} AS object_key FROM review_analysis_jobs
     WHERE session_id = ?1 AND ${column} IS NOT NULL AND (?2 IS NULL OR id = ?2)
     ORDER BY created_at DESC, id DESC LIMIT 1`,
  ).bind(sessionId, audit?.run?.job_id ?? null).first<{ object_key: string }>();
  return row ? env.RECORDINGS.get(row.object_key) : null;
}
