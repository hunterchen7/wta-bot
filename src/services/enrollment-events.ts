...
export async function logEnrollmentEvent(env: Env, event: EnrollmentEventInput): Promise<void> {
  await env.DB.prepare(
    `INSERT OR IGNORE INTO enrollment_events
       (discord_id, discord_username, guild_id, event_type, source, external_id)
     VALUES (?1, ?2, ?3, ?4, ?5, ?6)`,
  ).bind(
    event.discordId,
...
export async function enrollmentFunnel(env: Env) {
  const [counts, people] = await Promise.all([
    env.DB.prepare(
      `SELECT
         count(DISTINCT CASE WHEN event_type = 'link_generated' THEN discord_id END) AS generated,
         sum(CASE WHEN event_type = 'link_generated' THEN 1 ELSE 0 END) AS links_issued,
         count(DISTINCT CASE WHEN event_type = 'form_opened' THEN discord_id END) AS opened,
         count(DISTINCT CASE WHEN event_type = 'enrollment_completed' THEN discord_id END) AS completed
       FROM enrollment_events`,
    ).first<{ generated: number; links_issued: number; opened: number; completed: number }>(),
    env.DB.prepare(
      `WITH activity AS (
         SELECT discord_id,
         ...
         FROM enrollment_events GROUP BY discord_id
       )
