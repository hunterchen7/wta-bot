...
export function optinButtons(weekId: number) {
  return buttonRow([
    ...
    { id: `optin:${weekId}:standby`, label: 'Set standby availability', style: 2 },
...
export async function openOptin(env: Env, week: Week): Promise<void> {
  ...
  const content =
    ...
    `**Standby availability** — choose how many extra sessions you can take in each role if someone needs a partner.\n` +
